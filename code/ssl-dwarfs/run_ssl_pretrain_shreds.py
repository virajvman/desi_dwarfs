...
/global/u1/v/virajvm/miniforge3/envs/ssl-pl/bin/python ssl-dwarfs/run_ssl_pretrain_shreds.py 
'''
...
if __name__ == '__main__':
    ...
    for image_type in ["recon","org"]:
        num_chunks = 6
        
        for file_i in range(num_chunks):
    
            h5_data_path = f"/pscratch/sd/v/virajvm/catalog_dr1_dwarfs/ssl_shred_data/h5_datasets/ssl_shred_{image_type}_p{file_i}.h5"
            print(h5_data_path)
            ...
            class Args: # In general codes in this project use argparse. Args() simplifies this for this example 
                ...
                num_nodes = 1
                ngals_tot = gals[f'images'].shape[0]
                # Training
                verbose = True
                ssl_training = True
                ...
                chunksize= ngals_tot
                batch_size_per_gpu = int(gals[f'images'].shape[0]/4)
                num_gpus = 1
                data_dim = 2048
                ...
                n_pca_components = 8
                n_umap_components = 2
                umap_embedding_file_path = os.path.join(out_dir, f"{umap_file_head}_{gals[f'images'].shape[0]}_embedding.npz")
                umap_transform_file_path = os.path.join(out_dir, f"{umap_file_head}_{gals[f'images'].shape[0]}_transform.pkl")
            ...
            save_rep = f'/pscratch/sd/v/virajvm/catalog_dr1_dwarfs/ssl_shred_data/representations/represent_chunk_{image_type}_{file_i}.npy'
