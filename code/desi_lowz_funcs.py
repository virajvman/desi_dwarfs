...
def read_vac_line_info(gal_cat,columns,which_vac = "SPECPHOT",coord_name = "", stack_with_org=False):
    ...
    The which_vac points to what extension to read from the iron fastspec VAC: FASTSPEC, SPECPHOT are the main ones of interest (the dwarf multi-extension catalog uses FASTSPEC for matched fastspec columns; the SPEC_DERIVED HDU there holds spectroscopically derived properties only).
    '''
    #VAC data upload
...
from plot_style import (
    MARGIN_LABEL, MARGIN_TICKS, MARGIN_PAD, MARGIN_SHARED, MARGIN_SPLIT, MARGIN_CBAR,
    apply_paper_style, reshape_axes, make_subplots,
)
...
def print_maskbit_summary(catalog,max_bitnum=19):

    maskbits = np.asarray(catalog['DWARF_MASKBIT'])  # replace `t` with your table name

    print(f"{'Bit':>4} {'Count':>10}")
    for bit in range(max_bitnum+1):  # bits 0 through 19
...
# conf_interval, plot_2d_dist, and make_alternating_plot now live in
# plot_style.py (lightweight: numpy/matplotlib/scipy only, no desispec
# dependency) so the catalog-paper figure scripts can import them without
# the full DESI stack. Re-exported here to preserve existing
# `from desi_lowz_funcs import ...` call sites.
from plot_style import conf_interval, plot_2d_dist, make_alternating_plot
