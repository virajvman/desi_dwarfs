...
if __name__ == '__main__':
    ...
    if only_deredshift == False:
        ...
        print("Flux array size = ", all_fluxs_out.nbytes / 1e9, "GB")
        print("Flux ivar array size = ",all_flux_ivars_out.nbytes / 1e9, "GB")

        Ngals = all_fluxs_out.shape[1]
        
        # ##convert this into cupy arrays
        # print("About to move flux to GPU", flush=True)
        # all_fluxs_out_cp = cp.array(all_fluxs_out)
        # print("Flux moved to GPU", flush=True)

        # print("About to move ivar to GPU", flush=True)
        # all_flux_ivars_out_cp = cp.array(all_flux_ivars_out)
        # print("Ivar moved to GPU", flush=True)
        ...
        if train_nnmf:
            ...
            n_sub = min(75000, Ngals)
            idx = rng.choice(Ngals, size=n_sub, replace=False)

            all_fluxs_out_cp_subsample = cp.array(all_fluxs_out[ : , idx ])
            all_flux_ivars_out_cp_subsample = cp.array(all_flux_ivars_out[: , idx  ])
        
            print(f"Shape of original spectra array = {all_fluxs_out.shape} ")
            ...
            W_nearly = np.load("/pscratch/sd/v/virajvm/catalog_dr1_dwarfs/iron_spectra/nnmf_templates/normalization_templates_dwarfs_v2.npy")
        ...
        print(f"scaling coeffs shape = {coeffs.shape}")
        ...
        all_fluxs_out =  all_fluxs_out * scaling_factors
        all_flux_ivars_out =  all_flux_ivars_out * (1/scaling_factors**2)
        ...
        print(np.shape(all_fluxs_out))
    
        all_fluxs_out = all_fluxs_out[ :,  (coeffs != 0) ]
        all_flux_ivars_out = all_flux_ivars_out[ :, (coeffs != 0) ]
    
        ##also removen the redshifts and tgids
        all_zreds = all_zreds[ coeffs != 0 ]
        all_tgids = all_tgids[ coeffs != 0 ]
        scaling_factors = scaling_factors[coeffs != 0]
    
        print("Scaled fluxs shape:", np.shape(all_fluxs_out))
        print("Zred shapes:", all_zreds.shape)
        ...
        if train_nnmf:
            ...
            #training set
            all_fluxs_train = all_fluxs_out[:,train_inds]
            all_flux_ivars_train = all_flux_ivars_out[:,train_inds]
            ...
            W_nearly = cp.array(np.load("/pscratch/sd/v/virajvm/catalog_dr1_dwarfs/iron_spectra/nnmf_templates/templates_dwarfs_v2.npy"))
        ...
        #TODO: make this such that code does not break if training is not being done
        ...
        save_final = "/pscratch/sd/v/virajvm/catalog_dr1_dwarfs/iron_spectra/spectra_files/desi_dr1_dwarf_catalog_nnmf.h5"
        ...
        with h5py.File(save_final, "w") as f:
            ...
            f.create_dataset("FLUX_NORM", data=all_fluxs_out, dtype='f4')
            f.create_dataset("FLUX_IVAR_NORM", data=all_flux_ivars_out, dtype='f4')
            ...
            #note that this valid array is only available if the we did do the train
