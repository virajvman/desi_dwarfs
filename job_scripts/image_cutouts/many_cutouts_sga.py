...
def cutout_one(jpegfile, ra, dec, dry_run, rank, iobj,cut_size):
    ...
    if dry_run:
        ...
        cutout(ra, dec, jpegfile, width=width, height=height, layer='ls-dr9', pixscale=0.262, force=False, bands = ["g","r","z"],invvar=False,maskbits=True)
...
def plan(comm=None,outdir_data='.', mp=1):
    ...
    out = Table.read("/pscratch/sd/v/virajvm/catalog_dr1_dwarfs/iron_sga_matched_dwarfs.fits") 
    ...
    for k in range(len(allobjids)):
        file_i = outdir_data + f"/image_tgid_{allobjids[k]:d}_{allra[k]:.3f}_{alldec[k]:3f}.fits"         
